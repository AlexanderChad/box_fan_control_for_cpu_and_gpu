...
При запуске будет печататься список найденых GPU, например:  
```
Запуск fanc_server...
Найдены GPU:
ID    | UUID                                          | Название                 | Производитель
-----------------------------------------------------------------------------------------------------
0     | GPU-8a03cf4b-5694-4a0e-aacb-7e2b9d89f611      | NVIDIA GeForce RTX 4060  | ASUS
1     | GPU-ccbf29bc-c76c-4af3-adac-584b364f35c7      | NVIDIA GeForce RTX 3080  | NVIDIA Ref.
-----------------------------------------------------------------------------------------------------
```

Если требуется задать свой профиль управления вентиляторами карты, то нужно в `fan_config.json` прописать карту и кривую.  
UUID, температура - обороты вентилятора. Пример:  
```
{"GPU-ccbf29bc-c76c-4af3-adac-584b364f35c7": [[50,30],[60,60],[70,100]]}
```
